...
// ---------- plant switcher (v4.36.0) ----------
function isLeader(){ return role==='owner'||role==='manager'; }
function renderPlantSwitch(){
  const host=$('#plantLabel'); if(!host) return;
  const wrap=$('#plantSwitch');
  if(!plant){ if(wrap) wrap.hidden=true; return; }
  host.textContent=plant.name||'';
  if(!wrap) return;
  // only offer the control when there is something to switch to
  if(allPlants.length<2){
    wrap.hidden=false;                 // still show the name, just not the arrow
    const ar=$('#plantBtn i'); if(ar) ar.style.display='none';
    $('#plantBtn').style.pointerEvents='none';
    return;
  }
  const ar=$('#plantBtn i'); if(ar) ar.style.display='';
  $('#plantBtn').style.pointerEvents='';
  wrap.hidden=false;
  host.style.cursor='pointer';
  $('#plantList').innerHTML=allPlants.map(p=>
    `<button type="button" class="plant-opt${p.id===plant.id?' on':''}" onclick="window.switchPlant('${p.id}')">
       <span>${esc(p.name)}</span>${p.id===plant.id?'<b>&#10003;</b>':''}
     </button>`).join('')
    + (isLeader() ? `<div class="pm-foot"><button type="button" class="plant-opt"
         onclick="window.togglePlantMenu(false);window.go('plants')">
         <span>&#9881; Manage plants</span></button></div>` : '');
}

window.togglePlantMenu=(force)=>{
  const m=$('#plantMenu'); if(!m) return;
  const open = force===undefined ? m.hidden : force;
  m.hidden=!open;
  $('#plantSwitch')?.classList.toggle('open',open);
};

window.switchPlant=async id=>{
  const next=allPlants.find(p=>p.id===id);
  window.togglePlantMenu(false);
  if(!next||next.id===plant?.id) return;
  plant=next;
  try{localStorage.setItem('pmPlantId',plant.id)}catch(_){}
  $('#plantLabel').textContent=plant.name||'';
  renderPlantSwitch();
  subscribe();                       // re-bind realtime to the new plant channel
  try{ await operations.audit('plant_switched','plant',plant.id,{name:plant.name}); }catch(_){}
  toast('Switched to '+plant.name);
  loadView();                        // reload the current module with the new plant
};

// close the menu on outside tap / Escape
document.addEventListener('pointerdown',e=>{
  const m=$('#plantMenu');
  if(m&&!m.hidden&&!e.target.closest('#plantSwitch')) window.togglePlantMenu(false);
},true);
document.addEventListener('keydown',e=>{ if(e.key==='Escape') window.togglePlantMenu(false); });

// ---------- plants & sites view (v4.36.0) ----------
async function plantsView(){
  $('#toolbar').hidden=true; $('#add').hidden=true;
  const leader=isLeader();
  const {data:rows}=await sb.from('plants').select('*')
  ...
  $('#content').innerHTML=`
    <div class="view-head">
      <h2>Plants &amp; Sites</h2>
      <p class="muted">Each plant keeps its own assets, work orders, stores and team.
      Switch between them from the plant name in the top bar.</p>
    </div>
    ${allPlants.map(p=>`
      <div class="plant-row">
        <div class="pr-main">
          <b>${esc(p.name)}</b>
          <small>${esc(p.location||'No location set')}</small>
        </div>
        ${p.id===plant?.id?'<span class="pr-cur">CURRENT</span>':''}
        <div class="pr-acts">
          ${p.id!==plant?.id?`<button onclick="window.switchPlant('${p.id}')">Open</button>`:''}
          ${leader?`<button onclick="window.editPlant('${p.id}')">Edit</button>`:''}
        </div>
      </div>`).join('')}
    ${leader?`<button class="btn-primary" style="width:100%;margin-top:8px"
        onclick="window.editPlant()">＋ Add a plant</button>`:''}
    ${!leader?'<p class="muted" style="margin-top:14px">Only an owner or manager can add or rename a plant.</p>':''}
  `;
}

window.editPlant=async id=>{
  const row=id?allPlants.find(p=>p.id===id):null;
  const m=$('#modal'),f=$('#recordForm');
  $('#modalTitle').textContent=row?'Edit plant':'Add a plant';
  $('#fields').innerHTML=`
    <div class="form-field wide"><label>Plant name *</label>
      <input name="name" required maxlength="80" value="${esc(row?.name||'')}"
             placeholder="e.g. Spinning Unit 2"></div>
    <div class="form-field wide"><label>Location</label>
      <input name="location" maxlength="120" value="${esc(row?.location||'')}"
             placeholder="e.g. Port Qasim, Karachi"></div>`;
  ...
  f.onsubmit=async e=>{
    ...
    if(!name) return;
    const btn=$('#saveRecord'); if(btn){btn.disabled=true;btn.textContent='Saving…';}
    try{
      if(row){
        const{error}=await sb.from('plants').update({name,location:fd.location||null}).eq('id',row.id);
        if(error) throw error;
        await operations.audit('plant_updated','plant',row.id,{name});
        if(plant?.id===row.id){plant.name=name;plant.location=fd.location||null;}
        toast('Plant updated');
      }else{
        const{data,error}=await sb.from('plants')
          .insert({organization_id:org.id,name,location:fd.location||null})
          .select().single();
        if(error) throw error;
        await operations.audit('plant_created','plant',data.id,{name});
        toast('Plant added');
      }
      m.close(); loadView();
    }catch(err){
      toast(/row-level security|permission/i.test(err.message||'')
        ? 'Not allowed. Ask the account owner to add the plant.'
        : 'Could not save: '+(err.message||'unknown error'));
    }finally{
      const b=$('#saveRecord'); if(b){b.disabled=false;b.textContent='Save';}
    }
  };
  m.showModal();
};
...
async function renderCompanyProfile(){
  ...
  $('#content').innerHTML=`<section class="module-head"><div><h1>Company Profile & Theme</h1><p>Changes apply immediately to this company workspace and document branding.</p></div><button class="secondary" onclick="window.go('dashboard')">Dashboard</button></section><div class="card profile-shell"><div class="logo-panel"><div id="companyLogoPreview" class="logo-preview">${logo?`<img src="${logo}" alt="Company logo">`:'<span>No logo uploaded</span>'}</div><label class="form-field"><span>Company logo</span><input id="setLogo" type="file" accept="image/png,image/jpeg,image/webp"></label></div><form id="companyProfileForm" class="profile-form"><label class="form-field"><span>Application name</span><input id="setApp" value="${esc(x.app_name||'PlantMaster Pro')}" required></label><label class="form-field"><span>Plant display name</span><input id="setPlant" value="${esc(x.plant_display_name||plant.name)}" required></label><label class="form-field color-field"><span>Primary colour</span><input id="setPrimary" type="color" value="${esc(x.primary_color||'#0ea5e9')}"></label><label class="form-field color-field"><span>Secondary colour</span><input id="setSecondary" type="color" value="${esc(x.secondary_color||'#22d3ee')}"></label><label class="form-field"><span>Theme</span><select id="setTheme">${opts(['industrial','corporate','green','steel','light'],x.theme||'industrial')}</select></label><label class="form-field"><span>Pattern</span><select id="setPattern">${opts(['grid','dots','diagonal','circuit','none'],x.pattern||'grid')}</select></label><label class="form-field span-2"><span>Company address</span><textarea id="setAddress">${esc(x.address||'')}</textarea></label><label class="form-field"><span>Contact number</span><input id="setContact" value="${esc(x.contact||'')}"></label><label class="form-field"><span>Company email</span><input id="setEmail" type="email" value="${esc(x.email||'')}"></label><label class="form-field"><span>Daily report time</span><input id="setReportTime" type="time" value="${esc(String(x.report_time||'11:00').slice(0,5))}"></label><label class="form-field"><span>Document footer</span><input id="setFooter" value="${esc(x.footer_text||'')}"></label><div class="theme-preview"><b>Live theme preview</b><p>Buttons, highlights, patterns and documents use your selected company colours.</p></div><div class="span-2 actions"><button type="submit" class="primary">Save & Apply Profile</button></div></form></div>`;
...
async function saveCompanyProfile(e){
  e.preventDefault();const button=e.submitter;button.disabled=true;button.textContent='Saving…';try{let logo_path;const file=$('#setLogo').files[0];if(file){if(file.size>5*1024*1024)throw Error('Logo must be smaller than 5 MB');logo_path=`${org.id}/branding/${crypto.randomUUID()}-${file.name.replace(/[^\w.-]/g,'_')}`;const u=await sb.storage.from(FILE_BUCKET).upload(logo_path,file);if(u.error)throw u.error}
  const payload={organization_id:org.id,app_name:$('#setApp').value.trim(),plant_display_name:$('#setPlant').value.trim(),primary_color:$('#setPrimary').value,secondary_color:$('#setSecondary').value,theme:$('#setTheme').value,pattern:$('#setPattern').value,address:$('#setAddress').value.trim(),contact:$('#setContact').value.trim(),email:$('#setEmail').value.trim(),report_time:$('#setReportTime').value||'11:00',footer_text:$('#setFooter').value.trim(),updated_at:new Date().toISOString()};if(logo_path)payload.logo_path=logo_path;const r=await sb.from('organization_settings').upsert(payload);if(r.error)throw r.error;await applyBranding(payload);await operations.audit('company_profile_updated','organization',org.id,{theme:payload.theme,pattern:payload.pattern});toast('Company profile saved and applied');await renderCompanyProfile()}catch(error){toast(error.message)}finally{button.disabled=false;button.textContent='Save & Apply Profile'}}

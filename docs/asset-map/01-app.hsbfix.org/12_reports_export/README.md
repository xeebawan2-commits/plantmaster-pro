...
_Export any module for a date range as CSV, Excel, Word or PDF._

## 1. App section / UI
| File | Role |
|---|---|
| `app.js` | Report module + export builders |

## 2. Schema / SQL
Full DDL for this section is in **`schema.sql`** (extracted from the sources below).

**Tables:** `work_orders`, `assets`, `spares`, `daily_logs`, `attendance`

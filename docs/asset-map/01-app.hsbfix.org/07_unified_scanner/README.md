...
_QR/barcode/document/nameplate scanning linking a machine to its record & manuals._

## 1. App section / UI
| File | Role |
|---|---|
| `scanner.js` | Scanner UI (QR, document, nameplate) |
| `scanner-v4.8.css` | Styling |

## 2. Schema / SQL
Full DDL for this section is in **`schema.sql`** (extracted from the sources below).

**Tables:** `scan_sessions`, `scan_pages`, `code_registry`, `assets`, `spares`, `tools`, `manuals`, `file_metadata`, `document_chunks`

**RPCs / functions:** `reserve_storage_upload()`, `finalize_storage_upload()`, `cancel_storage_reservation()`

**Defined in:** 0002_operations.sql, 0003_inventory_procurement.sql, 0004_knowledge_files.sql, 0006_rpc.sql

## 3. Edge function
See **`edge-function.md`**. Functions: `vision-scanner`
